...
*   **`matches` Table:** The core table for game instances, using a `ulid` for public reference and a flexible `game_state` JSON column. It will also include a `last_move_at` timestamp to handle turn timeouts.
*   **Gamification Tables:** A suite of tables including `point_ledgers`, `global_ranks`, `badges`, `user_badge`, and `user_game_levels` will manage all aspects of the gamification system.
...
*   **AI Logic:** The `getAIMove` method within each game's service class will use the `ai_logic_path` on the agent's profile to instantiate the correct internal strategy class (e.g., `ValidateFourMinimax`, `CheckersHeuristic`).
...
*   **"Accept Match" Flow:** To prevent AFK players, a `MatchFound` event is dispatched via Reverb. Both clients must confirm within 10 seconds before the match is created.
...
*   **Turn Timeout:** A scheduled job will run every minute to check for active matches that have exceeded their `move_timeout_seconds`. Timed-out matches will be forfeited.

#### 7. Gamification System

A multi-layered gamification system will drive engagement and retention.

*   **Points & Ranks:** Users earn points from matches, which determine a `GlobalRank`.
*   **Levels & XP:** Users gain game-specific XP to increase their `Level` in each game, which is used for matchmaking.
*   **Badges:** Permanent, one-time achievements for reaching specific milestones.
*   **Level Decay:** A scheduled task will reduce a user's `Level` in a specific game if they are inactive for a set period, encouraging continued play.

#### 8. API Endpoints

The API will expose a comprehensive set of RESTful endpoints, including:
*   **Identity:** `/v1/sessions`, `/v1/user`
*   **Gameplay:** `/v1/games`, `/v1/matches`, `/v1/matches/{ulid}/moves`
...
A comprehensive admin panel will be built using **Filament v4** to manage users, agents, matches, billing, content (avatars), and platform-wide analytics.

#### 10. Future Expansion

The architecture is designed to accommodate future features outlined in `expansion.md`, including:
*   **Social Features:** Friends lists and real-time match chat.
*   **Gameplay Variety:** Tournaments and asynchronous (turn-based) gameplay.
*   **Monetization:** An in-game store for cosmetics and a ticket-based (pay-per-match) system.
*   **Team Play:** Party/group queuing for team-based games.

...
class PopOutHandler implements GameActionHandlerInterface
{
    public function validate(object $gameState, object $action): ValidationResult
    {
        if (! ($gameState instanceof ValidateFourBoard)) {
            return ValidationResult::invalid('INVALID_STATE_TYPE', 'Game state must be a ValidateFourBoard instance');
        }
    ...
    {
        if (! ($gameState instanceof ValidateFourBoard) || ! ($action instanceof PopOut)) {
            return $gameState;
        }
    ...
    {
        if (! ($state instanceof ValidateFourBoard)) {
            return [];
        }

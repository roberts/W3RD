 ...
 * $gameState = ValidateFourValidateFourBoard::fromArray($game->game_state);
 * ```
 *
 * @see \App\Interfaces\GameTitleContract For the interface all game modes must implement
 */
final class ValidateFourBoard extends BaseGameState
{
    /**
     ...
     *
     * @param  array<string, ValidateFourPlayer>  $players  Map of player ULID to ValidateFourPlayer
     ...
     * Example:
     * ```php
     * $gameState = ValidateFourValidateFourBoard::createNew(
    ...
    public static function createNew(
        ...
        $players = [
            $playerOneUlid => new ValidateFourPlayer(
                ulid: $playerOneUlid,
                position: 1,
                color: 'red'
            ),
            $playerTwoUlid => new ValidateFourPlayer(
                ulid: $playerTwoUlid,
                position: 2,
                color: 'yellow'
            ),
    ...
    public static function fromArray(array $stateData): static
    {
        ...
        foreach ($stateData['players'] ?? [] as $ulid => $playerData) {
            $players[$ulid] = ValidateFourPlayer::fromArray($playerData);
        }
